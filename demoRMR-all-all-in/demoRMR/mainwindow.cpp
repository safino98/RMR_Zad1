    ...
    ui(new Ui::MainWindow)
{
    ...
    ipaddress="192.168.1.12";//"192.168.1.14";//"127.0.0.1";//192.168.1.11toto je na niektory realny robot.. na lokal budete davat "127.0.0.1"
...
// Tu zadaj zelanu poziciu
    ...
    if(go){
        ...
        pa = atan2(diffY,diffX);
        if((pa-phi)>(PI/25.0) || (pa-phi)<(-PI/25.0)){
            robot.setRotationSpeed(((pa)-phi)*P1);

        }
        else{
            double speed = ((abs(diffX-x)+abs(diffY-y)))*P2;
            if(speed>200){
               speed = 200;
                }
            if(speed<50){
        ...
        if(abs(diffX) < 5.0 && abs(diffY) < 5.0){
            ...
            printf("\n\nRobot sa presuval do pozicie x= %f y= %f", targetX, targetY);
...
{
    printf("\n\nRobot sa presuva do pozicie x= %f y= %f", targetX, targetY);
...
{
robot.setRotationSpeed(3.14159);
